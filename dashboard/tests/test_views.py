...
class CategoryTestCase(TestCase):
    ...
    """Проверка views функций Category."""
...
class AuthenticationTestCase(TestCase):
    ...
    """Проверка аутентификации пользователя. Получение категории по id, всех категорий, удаление категорий доступно только авторизированным пользователям."""
    ...
    def test_get_categories_authentication(self):
        
        response = self.client.get(reverse('index'))
        self.assertRedirects(response, '/accounts/login/?next=/')

    def test_delete_category_authentication(self):
    ...
    """Проверка аутентификации пользователя. Получение отчета по расходам доступно только авторизированным пользователям."""
    
    def test_get_report_authentication(self):
        self.client = Client()
        self.client.logout()
        response = self.client.get(reverse('get_report'))
        self.assertRedirects(response, '/accounts/login/?next=/get_report')

    """Проверка аутентификации пользователя. Удаление расхода доступно только авторизированным пользователям."""

    def test_delete_expense_authentication(self):
        self.client = Client()
        self.client.logout()
        response = self.client.get(
            reverse('delete_expense', kwargs={'pk': self.category.pk}))
        self.client.delete(response)
        self.assertRedirects(
